...
class MWGCN(nn.Module):
        ...
        for text_i, asp_i in zip(range(len(texts)), range(len(asps))):
            ...
            for i in range(1, np.count_nonzero(texts[text_i]) - 1):
                if abs(i - asp_avg_index) + asp_len / 2 > self.opt.SRD:
                    ...
                    # distances[i] = 0 #CDM
                    # distances[i] = 1 #全
                    distances[i] = 1 - (abs(i - asp_avg_index) + asp_len / 2 - self.opt.SRD) / np.count_nonzero(
                        texts[text_i])  # 1/n
                else:
                    distances[i] = 1
    ...
    def forward(self, inputs):
        ...
        last = text_out[:, -1, :]
        weighted_last = self.alpha(last, text_out)

        #         直接从lstm得到的特征向量中提取方面词的特征
        aspect_mask = self.mask(text_out, aspect_double_idx)
        weighted_aspect = self.alpha(aspect_mask, text_out)

        # 在每层gcn使用相似度赋予权重

        # print(weighted_aspect.shape, 'aspect的尺寸')
        weighted_out = 0.6 * weighted_last + 0.4 * weighted_aspect#将两种权重使用一个可调节参数进行合并#使用0.6和0.4
